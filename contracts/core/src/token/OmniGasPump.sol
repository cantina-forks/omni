...
contract OmniGasPump is XAppUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event Refilled(address indexed recipient, uint256 paid, uint256 fee, uint256 cut, uint256 amtOMNI);
    ...
    /// @notice Map address to total historical fillUps(), denominated in OMNI.
    ...
    function fillUp(address recipient) public payable whenNotPaused returns (uint256) {
        ...
        bought[recipient] += amtOMNI;

        xcall({
            destChainId: omniChainId(),
            to: fund,
            conf: ConfLevel.Latest,
            data: abi.encodeCall(OmniXFund.settleUp, (recipient, bought[recipient])),
            gasLimit: SETTLE_GAS
        });

        emit Refilled(recipient, msg.value, f, cut, amtOMNI);
